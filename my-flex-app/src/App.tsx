        ...
        <div className='flex flex-row grow'>
          ...
          <div className={`p-1 flex ${containerSetting.direction == 'flex-row' ? 'flex-row' : 'flex-col'} ${containerSetting.isShowScrollbar === true ? 'overflow-auto' : ''}  h-[calc(100vh-3.75rem)] grow`}>
            <div style={{ gap: `${containerSetting.gap}px` }} className={`flex ${containerSetting.direction} ${containerSetting.alignItems} ${containerSetting.justifyContent} grow dark:bg-gray-950 bg-neutral-200 rounded-sm`}>
              {
                items.map((x, i) => (
                  <div key={i} style={{ width: `${pixelConvert(x.width, 'auto')}`, minWidth: `${pixelConvert(x.minWidth, 'auto')}`, maxWidth: `${pixelConvert(x.maxWidth, 'none')}`, flexGrow: `${x.grow}`, flexShrink: `${x.shrink}`, flexBasis: `${pixelConvert(x.basis, 'auto')}` }} className={`dark:bg-gray-800 bg-white border-neutral-400 flex ${x.alignSelf} p-0.5 border rounded-sm dark:border-gray-600`} >
                    ...
                    <div onClick={() => editItem(x)} className={`flex items-center fill-gray-950 w-10 ${item === x ? "dark:bg-gray-600 bg-neutral-200" : ""}  dark:hover:bg-gray-600`}>
                      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M20.71,7.04C21.1,6.65 21.1,6 20.71,5.63L18.37,3.29C18,2.9 17.35,2.9 16.96,3.29L15.12,5.12L18.87,8.87M3,17.25V21H6.75L17.81,9.93L14.06,6.18L3,17.25Z" /></svg>
                    </div>
                    <div className="border-r dark:border-gray-600 border-neutral-400"></div>
                    <div onClick={() => removeItem(x)} className='flex items-center w-10 fill-gray-950 dark:hover:bg-gray-600'>
              ...
              <div className={`dark:bg-gray-800 w-35 overflow-hidden bg-white border-neutral-400 flex  p-0.5 border rounded-sm dark:border-gray-600`} >
